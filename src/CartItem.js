...
const CartItem = ({ id, img, title, price, amount }) => {
  const {removeItem,controlAmount} =useGlobalContext();
  const handleAmount = (e)=>{
    const arrow = e.target.className.baseVal;
    if(arrow === "up"){
      controlAmount(id,"inc")
    }else{
      return controlAmount(id,"dec")
    }
  }
  return (
    <article className='cart-item'>
      ...
      <div onClick={handleAmount}>
        ...
        {/* decrease amount */}
        <button className='amount-btn down'>
